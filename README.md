...
A node.js(express) API for a blog can be comsumed with any frontend framework 
...
```bash
  git clone https://github.com/Ibezimmichael/node-blog-api
```
...
Start the server 
nodemon is requred as a devdependency
...
Copy the .env.exmple into your .env
...
```http
GET /api/users/profile-viewers/:id
```
...
```http
PUT /api/users/admin-unblock/:id
```
